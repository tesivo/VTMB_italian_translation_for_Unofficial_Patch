...
---
## Traduzione della Plus Patch (opzionale):
La Unofficial Patch include anche una "Plus Patch", un pacchetto opzionale che aggiunge contenuti aggiuntivi rispetto alla patch base (nuove armi, oggetti, la campagna dei Cacciatori, ecc.). Se durante l'installazione della Unofficial Patch avete scelto di includere anche la Plus Patch, potete tradurla anche voi seguendo questi passaggi:
- Copiate e sovrascrivete i file della cartella `Plus_Local` di questo repository nella stessa cartella `Unofficial_Patch` in cui avete installato la patch base (non serve nessuna cartella o riga di comando diversa: la Plus si avvia con la stessa `-game Unofficial_Patch`).
- Se non avete installato la Plus Patch, potete ignorare questa cartella: la traduzione della patch base funziona comunque perfettamente da sola.
